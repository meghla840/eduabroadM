...
(function (window) {
  ...
  var EduAuth = {
    ...
    /* ---------------- Comments (reusable across any page) ----------------
     * pageId identifies which page/post the comment belongs to, e.g.
     * "home-hero", "blog-post-5-steps-to-a-winning-sop", "contact".
     * Comments are auto-approved so they show instantly on the page; the
     * admin dashboard can still hide (unapprove) or delete abusive ones.
     */
    submitComment: function (payload) {
      ...
      log(db, "New comment on \"" + (payload.pageTitle || payload.pageId || "site") + "\" from " + (payload.name || "guest") + ".");
      writeDB(db);
      return comment;
    },

    commentsForPage: function (pageId) {
      return readDB().comments
        .filter(function (c) { return c.pageId === pageId && c.approved !== false; })
    ...
    setCommentApproved: function (commentId, approved) {
      var db = readDB();
      var c = db.comments.find(function (x) { return x.id === commentId; });
